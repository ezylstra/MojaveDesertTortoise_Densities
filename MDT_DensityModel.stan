...
parameters {
  ...
  vector[n_segments]      seg_l_raw;       // segment-level random effects in abundance model
...
transformed parameters {
  ...
  // Tortoise density: covariate and random effects in segment-level model
  for (i in 1:n_segments) 
    seg_l_mu[i] = recov_l[recovID[i],year[i]];
	for (i in 1:n_segments)
		seg_l[i] = seg_l_mu[i] + cov_lam[i] * beta_lam + sd_tran * tran_l_raw[transectID[i]] + sd_seg * seg_l_raw[i]; 
